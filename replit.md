...
1. **Yandex.Direct Ads** - Creates advertising headlines and text for Russia's leading search advertising platform
2. **Email & Social Media** - Generates content for email campaigns and social media posts (Instagram, VK, Telegram)
3. **Loyalty Programs** - Creates personalized messages for customer retention (birthday greetings, reactivation campaigns, special offers)
...
1. Client submits form data to generation endpoint
2. Server validates request using Zod schemas
3. Constructs prompt specific to the module
4. Calls YandexGPT API via OpenAI-compatible client
5. Parses AI response into structured format
6. Saves generation to database
